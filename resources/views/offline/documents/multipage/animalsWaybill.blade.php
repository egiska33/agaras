<table class="no-border">
	<tr>
		...
		<td class="font-5 text-right" width="65%">
			Forma patvirtinta Valstybinės maisto ir veterinarijos tarnybos direktoriaus 2006 m. kovo 16 d. įsakymu Nr. B1-207<br />
			(Valstybinės maisto ir veterinarijos tarnybos direktoriaus 2015 m. gegužės 19 d. įsakymo Nr. B1-433 redakcija)
		</td>
	</tr>
</table>
<table class="no-border">
	...
	<tr>
		<td>{{ \Carbon\Carbon::now()->format('Y') }} m. {{ \Carbon\Carbon::now()->format('m') }} mėn. {{ \Carbon\Carbon::now()->format('d') }} d.</td>
	</tr>
</table>
<table class="no-border no-collapse table-text-left margin-10">
	...
	<tr>
		<td colspan="2"	class="text-center text-below">(vardas, pavardė / įmonės pavadinimas, kodas, veterinarinio patvirtinimo Nr.*)</td>
	</tr>
</table>
<table class="no-border no-collapse table-text-left margin-10">
	<tr>
		<td>Gyvūnų laikytojo adresas</td>
		<td class="solid-line" width="76%">{{ $documentInfo['seller_address'] }}</td>
	</tr>
	<tr>
		<td colspan="2"	class="text-center text-below">(rajonas, savivaldybė, seniūnija, vietovė, gatvė, namo numeris)</td>
	</tr>
</table>
<table class="no-border no-collapse table-text-left">
	<tr>
		<td>Gyvūnų laikymo vieta</td>
		<td class="solid-line" width="80%">{{ $animalsHeldPlace }}</td>
	</tr>
	<tr>
...
<table class="no-border no-collapse table-text-left margin-10">
	<tr>
		...
		<td class="solid-line" width="47%">{{ $documentInfo['user_plate'] }} @if ((!empty($documentInfo['user_plate'])) && (!empty($documentInfo['user_trailer_plates']))) , @endif {{ $documentInfo['user_trailer_plates'] }}</td>
	</tr>
	<tr>
		<td></td>
...
<table class="no-border no-collapse table-text-left margin-10">
	<tr>
		...
		<td>Gyvūnų rūšis</td>
		<td class="solid-line" width="15%">{{ $documentInfo['doc__v_g']['sex'] }}</td>
	</tr>
</table>
...
<p class="no-margin">
	Gyvunų skaičius/kiekis (suskaičiuoti list'o elementus) vnt. / kg
</p>
<table class="no-border no-collapse table-text-left margin-10">
	<tr>
		<td>Aš,</td>
		<td class="solid-line" width="55%">{{ $documentInfo['seller_name'] }}</td>
		<td width="43%">, p a t v i r t i n u, kad:</td>
	</tr>
	<tr>
		<td></td>
		<td class="text-center text-below">(gyvūnų laikytojo vardas, pavardė / įmonei atstovaujančio asmens vardas ir pavardė)</td>
		<td></td>
	</tr>
</table>
<ol class="list-rules">
	<li>
		Gyvūnų laikymo vietoje išvežamiems gyvūnams netaikomi / taikomi (nereikalingą žodį išbraukti) pagal
		<span class="solid-line" style="width: 50%; height: 10px; display: inline-block;"></span>
		apribojimai dėl gyvūnų užkrečiamųjų ligų;
	</li>
	<li>
		Arklinių šeimos gyvūnai neturėjo sąlyčio su arklinių šeimos gyvūnais, sergančiais užkrečiamąja liga, ne mažiau kaip 15 d.;
	</li>
	<li>
		Arklinių šeimos gyvūnai nebuvo / buvo (nereikalingą žodį išbraukti) gydomi paskutinius 6 mėnesius vadovaujantis 2006 m. gruodžio
	...
	<li>
		...
		<span class="solid-line" style="width: 45%; height: 10px; display: inline-block;"></span>
		, kurio (-ių) išlauka pasibaigė <span class="solid-line" style="width: 10%; height: 10px; display: inline-block;"></span>
	</li>
	<li>
		Minėti gyvūnai tinkami vežti nustatytu maršrutu laikantis 2004 m. gruodžio 22 d. Tarybos reglamento (EB) Nr. 1/2005 dėl gyvūnų
	...
	<li>
		Šis važtaraštis surašytas 2 (dviem) egzemplioriais, iš kurių vienas duodamas gyvūnų laikytojui, kitas – transporto priemonės vairuotojui,
